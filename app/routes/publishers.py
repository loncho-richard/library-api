...
async def create_publisher(publisher: PublisherCreate, service: PublisherService = Depends(get_publisher_service)):
    return service.create_publisher(publisher)

@router.get("/", response_model=list[PublisherRead])
async def list_publishers(service: PublisherService = Depends(get_publisher_service)):
    return service.get_publishers()

@router.get("/{publisher_id}", response_model=PublisherRead)
async def get_publisher(publisher_id: int, service: PublisherService = Depends(get_publisher_service)):
...
async def update_publisher(publisher_id: int, update: PublisherUpdate, service: PublisherService = Depends(get_publisher_service)):
...
async def delete_publisher(publisher_id: int, service: PublisherService = Depends(get_publisher_service)):
